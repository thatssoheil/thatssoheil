...
| **Dedicated `/ds` showcase route** | The homepage is the stable consumer and reference; a parallel gallery drifted from the real product. | A second route, client, or contributor needs a browsable catalog. |
...
| **Glass enrichment** (elevation tiers, grain, tint, chromatic aberration) | The current material is fill + shadow, with no visible dark-mode rim; extra effects are a legibility/perf risk. | A concrete surface earns it (own ADR). |
