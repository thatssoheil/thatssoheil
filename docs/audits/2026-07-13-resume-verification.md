...
- `pnpm start`: the production server reached `Ready in 179ms` at `http://localhost:3000`; all browser and PDF evidence below was captured from that production process.
...
`stat -c '%s bytes'` reported `107245 bytes`, which is below the `1048576`-byte limit.
